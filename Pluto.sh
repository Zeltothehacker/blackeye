...
menu() {
    while true; do
        banner
        printf "%s╔═───────────────────────────────────────────────────────═╗ %s\n" "$Y" "$N"
        printf "%s║%s                    PHISHING PAGES                       %s║%s\n" "$Y" "$N" "$Y"  
        printf "%s╠═───────────────────────────────────────────────────────═╣%s\n" "$Y" "$N"    
        printf "%s║%s [01]%s Instagram  %s[02]%s Facebook   %s[03]%s Google             %s║%s\n" "$Y" "$G" "$N" "$G" "$N" "$G" "$N" "$Y"
        printf "%s║%s [04]%s Microsoft  %s[05]%s Netflix    %s[06]%s PayPal             %s║%s\n" "$Y" "$G" "$N" "$G" "$N" "$G" "$N" "$Y"
        printf "%s║%s [07]%s Steam      %s[08]%s Twitter    %s[09]%s Spotify            %s║%s\n" "$Y" "$G" "$N" "$G" "$N" "$G" "$N" "$Y"
        printf "%s║%s [10]%s Adobe      %s[11]%s Badoo      %s[12]%s Crypto             %s║%s\n" "$Y" "$G" "$N" "$G" "$N" "$G" "$N" "$Y"
        printf "%s║%s [13]%s DeviantArt %s[14]%s Dropbox    %s[15]%s GitHub             %s║%s\n" "$Y" "$G" "$N" "$G" "$N" "$G" "$N" "$Y"
        printf "%s║%s [16]%s GitLab     %s[17]%s LinkedIn   %s[18]%s Messenger          %s║%s\n" "$Y" "$G" "$N" "$G" "$N" "$G" "$N" "$Y"
        printf "%s║%s [19]%s MySpace    %s[20]%s Origin     %s[21]%s Pinterest          %s║%s\n" "$Y" "$G" "$N" "$G" "$N" "$G" "$N" "$Y"
        printf "%s║%s [22]%s ProtonMail %s[23]%s Shopify    %s[24]%s Snapchat           %s║%s\n" "$Y" "$G" "$N" "$G" "$N" "$G" "$N" "$Y"
        printf "%s║%s [25]%s Shopping   %s[26]%s Twitch     %s[27]%s Verizon            %s║%s\n" "$Y" "$G" "$N" "$G" "$N" "$G" "$N" "$Y"
        printf "%s║%s [28]%s VK         %s[29]%s WordPress  %s[30]%s Yahoo              %s║%s\n" "$Y" "$G" "$N" "$G" "$N" "$G" "$N" "$Y"
        printf "%s║%s [31]%s Yandex     %s[32]%s InstaFol   %s[33]%s Custom             %s║%s\n" "$Y" "$G" "$N" "$G" "$N" "$G" "$N" "$Y"
        printf "%s╚─────────────────────────────────────────────────────────╝%s\n\n" "$Y" "$N"
